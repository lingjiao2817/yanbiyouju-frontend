...
def rename_check(check_id: int, title: str) -> None:
    db = get_db()
    db.execute("UPDATE checks SET title = ? WHERE id = ?", (title, check_id))
    db.commit()


def delete_check(check_id: int) -> bool:
    db = get_db()
    cursor = db.execute("DELETE FROM checks WHERE id = ?", (check_id,))
    db.commit()
...
def upgrade_result_payload(check_id: int, input_text: str, result: dict) -> dict:
    ...
    # 新格式包含 paragraphs + meta；旧格式（言必有据）包含 flags/verification_routes
    missing_new_fields = "paragraphs" not in result or "meta" not in result
